...
定义 batch tier（set 2）dynamic UBO 的打包与绑定语义：descriptor 稳定绑定固定窗口、offset 对齐取自 device capability、pack offset 直接作为 dynamic offset；in-flight 安全由 `DeviceFrameContext` 维护 per-inflight-frame 的 packed pool。
