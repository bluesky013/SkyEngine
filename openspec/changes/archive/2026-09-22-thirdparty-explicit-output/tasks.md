...
- [x] 1.3 (Optional) Add an explicit opt-in packaging command (`--archive <path>`) that does not mutate tracked files
...
- [x] 3.3 Update docs / build scripts that describe the third-party workflow

## 4. Validation

- [x] 4.1 Run the tool with an explicit output directory and confirm `cmake/thirdparty.json` is unchanged
- [x] 4.2 Configure + build the engine against that output
