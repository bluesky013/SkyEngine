...
在 aurora/core 内提供高于裸 `rhi::Buffer`/`rhi::Image` 的资源封装层：统一 `RenderResource` 基类（buffer 与 image 的公共基类，惰性创建 + 上传）、`StaticBuffer`/`DynamicBuffer`/`TransientBuffer` 三档存储 tier 模板，以及带解释元数据的 `VertexBuffer` / `IndexBuffer` / `UniformBuffer` / 通用 `StorageBuffer`。资源/显存统计归后端（VMA/D3D12MA），不在此能力内。
