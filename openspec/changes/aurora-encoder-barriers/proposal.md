...
- 在 `GraphicsEncoder` / `ComputeEncoder` / `BlitEncoder` 三类 encoder 上各加 **`PipelineBarrier(const BarrierInfo&)`** 方法，接受一组 image + buffer + memory barrier
...
- 4 个后端落地：
  - Vulkan：`vkCmdPipelineBarrier2`（sync2，VK 1.3 已强制）
  - DX12：`ResourceBarrier` + 必要时插入 UAV barrier
  - Metal：`MTLResource` 上的 `useResource:` + 多 encoder 之间的隐式同步；显式 barrier 用 `MTLBlitCommandEncoder::waitForFence` / `MTLFence`
  - GLES：`glMemoryBarrier` + `glTextureBarrier`（如可用）
