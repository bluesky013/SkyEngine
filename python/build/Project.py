...
class ProjectBuilder:
    ...
    def config_native(self):
        ...
        # set cmake lists
        cmake_native_template_path = os.path.join(self.enginePath, 'cmake', 'template', 'CMakeLists_native.txt')
        cmake_template_path = os.path.join(self.enginePath, 'cmake', 'template', 'CMakeLists_project.txt')
        with open(cmake_template_path, "r") as f:
            data = f.read()

        cmake_native_target_path = os.path.join(native_path, 'CMakeLists.txt')
        cmake_target_path = os.path.join(self.projectPath, 'CMakeLists.txt')
        if os.path.exists(cmake_target_path):
            os.remove(cmake_target_path)

        data = data.replace('@PROJECT_NAME@', '%s' % os.path.basename(self.projectPath))
        with open(cmake_target_path, 'w') as f:
            f.write(data)

        shutil.copy(cmake_native_template_path, cmake_native_target_path)
    ...
    def init_project_directory(self):
        ...
        path_list = [(asset_path, self.config_asset),
                     (config_path, self.config_config),
