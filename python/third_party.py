...
def get_platform_output_root():
    return os.path.join(args.output, args.platform)


def get_platform_metadata_path():
...
def write_cmake_cache():
    cache_path = os.path.join(args.output, THIRD_PARTY_CACHE_FILE)
    output_root = normalize_cmake_path(args.output)
    platform_output_path = normalize_cmake_path(get_platform_output_root())
    engine_path = normalize_cmake_path(args.engine)

    Path(args.output).mkdir(parents=True, exist_ok=True)
    with open(cache_path, 'w', encoding='utf-8') as f:
        f.write('# Auto-generated by python/third_party.py\n')
        f.write('# Do not edit manually.\n')
        f.write(f'set(SKY_THIRD_PARTY_PLATFORM "{args.platform}" CACHE STRING "SkyEngine third-party platform" FORCE)\n')
        f.write(f'set(SKY_THIRD_PARTY_ENGINE_ROOT "{engine_path}" CACHE PATH "SkyEngine root" FORCE)\n')
        f.write(f'set(SKY_THIRD_PARTY_OUTPUT_ROOT "{output_root}" CACHE PATH "SkyEngine third-party output root" FORCE)\n')
...
def save_build_metadata(metadata):
    meta_path = get_platform_metadata_path()
    Path(get_platform_output_root()).mkdir(parents=True, exist_ok=True)
    with open(meta_path, 'w', encoding='utf-8') as f:
        json.dump(metadata, f, indent=2, ensure_ascii=False)


def run_git_with_retry(label, operation):
    last_error = None

    for attempt, delay in enumerate([0, *GIT_RETRY_DELAYS], start=1):
        if delay > 0:
            print(f"[retry] {label} attempt {attempt}/{len(GIT_RETRY_DELAYS) + 1} after {delay}s")
            time.sleep(delay)
        ...
        try:
            ...
            print(f"[git] {label} failed on attempt {attempt}/{len(GIT_RETRY_DELAYS) + 1}: {stderr}")

    if last_error is None:
        raise RuntimeError(f"{label} failed without a captured git error")

    raise last_error


def is_commit_hash(ref):
    return bool(ref) and COMMIT_HASH_RE.fullmatch(ref) is not None
...
def copy_package(name, install_dir, build_type):
    ...
    platform_output_root = get_platform_output_root()

    dst_inc_path = os.path.join(platform_output_root, name, "include")
    dst_src_path = os.path.join(platform_output_root, name, "src")
    dst_lib_path = os.path.join(platform_output_root, name, "lib", build_type)
    dst_bin_path = os.path.join(platform_output_root, name, "bin")
...
def fill_common_options(options, build_type, install_dir):
    ...
    options['3RD_PATH'] = get_platform_output_root()
...
def process_package(package):
    ...
    if commit and not is_commit_hash(commit):
        raise ValueError(f"Commit '{commit}' for package '{name}' is not a full 40-character git commit id")

    if not os.path.exists(clone_dir):
        clone_kwargs = {}
        if not need_full_history:
            clone_kwargs['depth'] = 1
            if tag:
                clone_kwargs['branch'] = tag
        repo = run_git_with_retry(f"clone {name}", lambda: Repo.clone_from(url, str(clone_dir), **clone_kwargs))
    else:
    ...
    if commit:
        run_git_with_retry(f"fetch commit for {name}", lambda: repo.git.fetch('--depth=1', 'origin', commit))
        repo.git.checkout('--detach', 'FETCH_HEAD')
    elif tag and need_full_history:
        run_git_with_retry(f"fetch tags for {name}", lambda: repo.git.fetch('--tags'))
