...
namespace sky::aurora {
    ...
    // flag bit
    // Stage-agnostic resource access classes (shader stage is expressed separately
    // via PipelineStageFlags in the barrier, not encoded in the access bit).
    ...
    enum class ImageViewUsageFlagBit : uint32_t {
        NONE                = 0x00000000,
        CUBE_MAP_COMPATIBLE = 0x00000001,
    };
    using ImageViewUsageFlags = Flags<ImageViewUsageFlagBit>;
    ENABLE_FLAG_BIT_OPERATOR(ImageViewUsageFlagBit)
