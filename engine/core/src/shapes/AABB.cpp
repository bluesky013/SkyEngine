...
namespace sky {
    ...
    AABB AABB::Transform(const AABB& box, const Matrix4 &mtx)
    {
        ...
        Vector3 min_c1 = ToVec3(mtx.m[0]) * box.min.x;
        Vector3 min_c2 = ToVec3(mtx.m[1]) * box.min.y;
        Vector3 min_c3 = ToVec3(mtx.m[2]) * box.min.z + ToVec3(mtx.m[3]); // place 4th column add here for free add (MAD)

        // compute column multiplies for the AABB max
        Vector3 max_c1 = ToVec3(mtx.m[0]) * box.max.x;
        Vector3 max_c2 = ToVec3(mtx.m[1]) * box.max.y;
        Vector3 max_c3 = ToVec3(mtx.m[2]) * box.max.z + ToVec3(mtx.m[3]); // place 4th column add here for free add (MAD)
